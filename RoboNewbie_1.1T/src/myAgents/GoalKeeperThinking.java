 ...
 */
public class GoalKeeperThinking {
     ...
     */
    public GoalKeeperThinking(PerceptorInput percIn, LocalFieldView localView,
    ...
    public void decide() {
        if (motion.ready()) {
            ...
            // if the robot has the actual ball coordinates
                ...
                // if the ball is close and in front of the goal
                else if (Math.abs(ballCoords.getY()) < 1 && ballCoords.getX() < 0.5) {
                    // If the ball is to the left, move left
                    if (ballCoords.getY() > 0) {
                        motion.setSideStepLeft();
                    }
                    // If the ball is to the right, move right
                    else if (ballCoords.getY() < 0) {
                        motion.setSideStepRight();
                    }
                }

                // if the ball is not close, stay in the goal area
                else {
                    if (robotIsWalking) {
                        motion.setStopWalking();
                        robotIsWalking = false;
                    } else {
                        // move side-to-side within the goal area
                        if (ballCoords.getY() > 0) {
                            motion.setSideStepLeft();
                        } else {
                            motion.setSideStepRight();
